  ...
  for( int i=0; i<parent->total_mapped_region; i++ ){
    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case STACK_SEGMENT:
        ...
        break;
      case HEAP_SEGMENT:{
        // COW fork: share heap pages between parent and child as read-only with COW flag
        // instead of copying them. added @lab3_challenge3
        ...
        // when mapping the heap blocks
        {
          ...
          // map heap blocks as COW (read-only + COW flag) for both parent and child
          for (uint64 heap_block = current->user_heap.heap_bottom;
               heap_block < current->user_heap.heap_top; heap_block += PGSIZE) {
            if (free_block_filter[(heap_block - heap_bottom) / PGSIZE])  // skip free blocks
              continue;

            // get parent's physical address for this heap block
            uint64 pa = lookup_pa(parent->pagetable, heap_block);

            // modify parent's PTE: remove write, add COW flag
            pte_t *parent_pte = page_walk(parent->pagetable, heap_block, 0);
            *parent_pte = (*parent_pte & ~PTE_W) | PTE_COW;

            // map same physical page into child as read-only + COW
